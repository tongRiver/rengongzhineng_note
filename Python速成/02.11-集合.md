...
因为集合是无序的，所以当集合中存在两个同样的元素的时候，Python只会保存其中的一个（**唯一性**）；同时为了确保其中不包含同样的元素，集合中放入的元素只能是**不可变**的对象（确定性）。
...
但是**创建空集合的时候只能用`set`来创建，因为在Python中`{}`创建的是一个空的字典**：
...
```
    ...
    <ipython-input-4-c0ded720561a> in <module>
    ----> 1 t.remove(10)
    KeyError: 10
...
```
    ...
    <ipython-input-8-9af808d05936> in <module>
          1 s = set()
          2 # 报错
    ----> 3 s.pop()
