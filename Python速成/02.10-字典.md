...
当我们 `print` 一个字典时，Python并不一定按照插入键值的先后顺序进行显示,因为字典中的键本身**不一定是有序的**。
...
因此，Python中**不能用数字索引按顺序查看字典中的值**，而且数字本身也有可能成为键值，这样会引起混淆：
...
出于hash的目的，Python中要求这些键值对的**键必须是不可变的，而值可以是任意的Python对象**。
...
synonyms['mutable'] = ['changeable', 'variable', 'varying', 'fluctuating',
                       ...
                       'fickle', 'uneven', 'unstable', 'protean']  # 键为字符串（不可变），值为列表（可变）
synonyms['immutable'] = ['fixed', 'set', 'rigid', 'inflexible', 
                         'permanent', 'established', 'carved in stone']  # 键为字符串（不可变），值为列表（可变）
synonyms
```
...
有时候，也可以使用**元组**作为键值，例如，可以用元组做键来表示从第一个城市飞往第二个城市航班数的多少：
...
```
删除**并返回字典中键 `key` 对应的值**，如果没有这个键，返回 `default` 指定的值（默认是 `None` ）。
...
修改：把'first'改成'James'，同时插入'middle'的值'Clerk'：
